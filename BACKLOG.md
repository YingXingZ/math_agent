...
- [x] 账户/角色、教师班级隔离、学生邀请码激活、管理员创建教师与审计基础。
- [x] 生产环境变量样例、仅暴露 8000 的 nginx 说明、SQLite 定时备份/恢复演练说明。
- [ ] 获得真实服务器、域名、DNS、HTTPS 证书签发权限后执行公网部署验收。
- [ ] 由学校确认学生数据、作业附件、成绩与审计记录的正式保存期限后，启用“预览→管理员确认”的清理流程。
