...
## 19. VLM 短暂故障的可恢复初评（2026-08-27）

- 当 18080 VLM / SSH 隧道返回 HTTP 502 或超时时，8000 会保留原作业并将结果安全地转入教师复核；这不是“学生作业未识别”的数学判断，也不会自动发布 0 分。
- 教师复核详情会明确显示 AI 服务错误，并提供“重新调用 AI 初评”。该操作只重跑同一份已归档原作业，不要求学生重新上传；结果仍须经过既有教师复核门禁。
- 运行时检查：本次演示提交 #3 的 `qwen_error` 为 `502 Bad Gateway`，且本机 `http://127.0.0.1:18080/health` 同样返回 502，故根因位于远端 VLM 服务/隧道上游，而非样张质量或题库答案。
- 后续修复了计算题类型的规范化使用及“单个等式右端 vs 标准最终值”的受限判等；它明确排除比较符、集合关系和多重等式。恢复后的同一演示提交重新验收为：`1+0=1`、10/10、confidence 1.00、无复核理由。
- 教师端每份作业现有“查看成绩”入口。待复核队列只保留不确定结果；该入口单列全部提交（含已自动批改的分数、识别摘要及原作业详情），避免成功初评从界面中不可见。
