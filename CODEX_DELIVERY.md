...
- 班级、取题组卷、打印、提交、教师确认和报表均通过；VLM 评分端点 `http://127.0.0.1:18080/grade-homework` 在本次演练返回 **502 Bad Gateway**。系统没有将 0 分自动发布，而是正确标记为 `review_required` 并要求教师确认，符合“不确定不自动评分”红线。
- 因此“自动批改的故障降级链路”已验证；“A100 VLM 成功识别并给出可用初评”的端到端验收仍阻塞于 18080 服务恢复，恢复后应重跑同一无痕沙箱演练。
