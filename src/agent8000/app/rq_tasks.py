...
def verify_provisional_question_bank_task(batch_id: int = 1) -> dict:
    """Second-pass verification through the authenticated 8014 HTTP contract.

    The RQ worker owns only the 8001 task state.  It never opens 8014's SQLite
    file, so this continues to work after the evidence service moves hosts.
    """
    import sqlite3
    import httpx
    from .config import settings
    from .evidence_client import headers as evidence_headers, url as evidence_url
    ...
    with httpx.Client(timeout=620, headers=evidence_headers()) as client:
        for row in rows:
            candidate_id = int(row["candidate_id"])
            baseline = "{}"
            decision, reason, second = "provisional", "", "{}"
            try:
                current_response = client.get(evidence_url(f"/answer-import-candidates/{candidate_id}"))
                current_response.raise_for_status()
                current = current_response.json()
                baseline = str(current.get("ai_review_json") or "{}")
                ai_response = client.post(evidence_url(f"/answer-import-candidates/{candidate_id}/ai-review"))
                ai_response.raise_for_status()
                after_response = client.get(evidence_url(f"/answer-import-candidates/{candidate_id}"))
                after_response.raise_for_status()
                after = after_response.json()
                second = str(after.get("ai_review_json") or "{}")
                ...
                if after.get("ptype") != "calc":
                    ...
                    review_response = client.post(evidence_url(f"/answer-import-candidates/{candidate_id}/review"), json=payload)
            ...
            except Exception as exc:  # individual candidate failures do not abort the batch
