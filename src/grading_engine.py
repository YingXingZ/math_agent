...
def normalize_expr(text: str) -> str:
    ...
    # Spaces inside a mathematical formula normally imply multiplication
    # (N x_0 e^{k N t}), not token concatenation (Nx0e^(kNt)).
    s = re.sub(r"(?<=[A-Za-z0-9_)])\s+(?=[A-Za-z0-9_(])", "*", s)
    s = re.sub(r"\s+", "", s)
    # 常见 LaTeX 片段还原
    # Accept one or more backslashes.  Browser/API payloads sometimes retain
    # a doubled slash (\\\\frac), which previously became a fake `frac(...)`
    # function and made an otherwise identical formula fail parsing.
    # One nesting level is enough for common forms such as
    # \frac{N x_0 e^{kNt}}{N-x_0+...}.
    s = re.sub(r"\\{1,}d?frac\{((?:[^{}]|\{[^{}]*\})*)\}\{((?:[^{}]|\{[^{}]*\})*)\}", r"((\1)/(\2))", s)
    ...
    # 去掉指数上的花括号: ^{2} -> ^2
    # Covers both e^{2} and e^{kNt}; parameter exponents are common in ODEs.
    s = re.sub(r'\^\{([^{}]+)\}', r'^(\1)', s)
    ...
    # Also remove labels such as x(t)= before comparing a final formula.
    s = re.sub(r"^[a-zA-Z][a-zA-Z0-9_]*(?:\([^=()]*\))?=", "", s)
    s = re.sub(r"_([0-9]+)", r"\1", s)
    # Textbook/OCR often removes multiplication marks in common model
    # parameters: kNt means k*N*t, not a new variable named kNt.
    s = re.sub(r"(?<![A-Za-z0-9_])kNt(?![A-Za-z0-9_])", "k*N*t", s)
    # In textbook notation `e^u` is Euler's exponential.  SymPy otherwise
    # treats lowercase e as a function token under implicit multiplication.
...
def to_sympy(text: str):
    ...
    if not s:
        return None
    try:
        # Supply a symbol-only local dictionary.  The parser's default
        # namespace contains names such as N/E that are SymPy functions,
        # while in textbook formulas they are usually parameters.
        names = set(re.findall(r"\b[A-Za-z][A-Za-z0-9_]*\b", s))
        reserved = {"sin", "cos", "tan", "cot", "sec", "csc", "log", "exp", "sqrt", "pi", "oo"}
        local_dict = {name: sp.Symbol(name) for name in names if name not in reserved}
        return parse_expr(s, transformations=TRANSFORMS, evaluate=True, local_dict=local_dict)
    except Exception:
        return None
